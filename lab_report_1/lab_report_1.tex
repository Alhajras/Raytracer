...
\begin{document}
	...
	\subsection{Rays}
	To generate an image using Raytracing, we must first cast a ray for each pixel in the image.
	Optical ray tracing describes a method for producing visual images constructed in 3D computer graphics environments, with more photorealism than ray casting or scanline rendering techniques. It works by tracing a path from an imaginary eye through each pixel in a virtual screen and calculating the object's color visible through it.
	All raytracers are using something called rays. Let’s think of a ray as a function \[\pmb{r} (t) = \pmb{o} + t\pmb{d} ,  \quad  0 < t < \infty \] Here $\pmb{} $ is a 3D position along a line
	...
	It is computing the intersection of a ray with a primitive such as a sphere is not tricky. However, since it is challenging to model most 3D objects with spheres alone, it is necessary to use some other types of primitive to represent more complex objects (objects of arbitrary shape).
	...
	The computer display is composed of pixels. Pixel is the smallest element of every digital image, and while modern computer monitors boast high resolutions that feature millions of pixels, these pixels are still rectangular. This means that when round shapes are shown on screen, you are almost guaranteed to see some jagged edges, that is, aliasing.
	Here we will use a basic MSAA that stands for "multisample anti-aliasing," which is among the most common types of anti-aliasing. It generally strikes the best balance between visual fidelity and performance. This can be implemented by averaging a bunch of samples inside each pixel.
	...
	Phong reflection is an empirical model of local illumination. It describes how a surface reflects light as a combination of the diffuse reflection of rough surfaces with the specular reflection of shiny surfaces. It is based on Phong's informal observation that shiny surfaces have small intense specular highlights, while dull surfaces have significant highlights that fall off more gradually. The model also includes an ambient term to account for the small amount of light that is scattered about the entire scene.
	\\
	...
	The amount of reflected vs. refracted light can be computed using what we call the \textit{Fresnel equations}, where $F_R$ is the reflected light portion, and $F_T$ is the portion transmitted through the material. 
	...
	A texture can be uniforms, such as a brick wall, or irregulars, such as wood grain or marble. The standard method is to create a 2D bitmapped image of the texture, called a "texture map," which is then "wrapped around" the 3D object. An alternate method is to compute the texture entirely via mathematics instead of bitmaps. Textures are so helpful; they reduce the geometric complexity of a scene by mapping a bit directly to an image or having a mathematical equation that can easily represent the surface color, they also reduce the number of vertices, and it reduces the modeling and rendering time. 
	...
	As described in the RIS documentation, in this case, we render the curve as a thin cylinder (see the rendered image above) or as a long string (like spaghetti), as shown. 
	...
	In this Raytracer, a Sphere BV is used; the reason is simple as Sphere has fast intersection text and is easy to generate and fit into the model or primitives. 
	...
	\begin{thebibliography}{9}
		...
		\bibitem{latexcompanion} 
		Stefan G.	\textit{"Collision queries using bounding box"}. University of north carolina.
